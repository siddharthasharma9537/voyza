...
type Nav = StackNavigationProp<OwnerNavigationParamList, 'Login'>;
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: C.cream }}>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={[styles.container, { flex: 1, justifyContent: 'center' }]}>
          <Text style={[FONT.h1, { marginBottom: 8 }]}>Voyza Host</Text>
          <Text style={[FONT.body, { marginBottom: 40 }]}>Sign in to manage your fleet</Text>
          {step === 1 ? (
            <>
              <Input label="Mobile Number" value={phone} onChangeText={setPhone} placeholder="+91 98765 43210" keyboardType="phone-pad" autoFocus />
              <Button label="Send OTP" onPress={handleSendOtp} loading={loading} style={{ marginTop: 8 }} />
              <Button label="Create an account" variant="outline" onPress={() => navigation.navigate('Register')} style={{ marginTop: 12 }} />
            </>
          ) : (
            <>
              ...
              <Button label="Verify & Sign In" onPress={handleVerify} loading={loading} style={{ marginTop: 8 }} />
